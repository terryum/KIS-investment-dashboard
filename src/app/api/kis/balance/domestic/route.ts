...
/**
 * GET /api/kis/balance/domestic
 * Domestic stock balance (TTTC8434R) across all accounts.
 */
  ...
  try {
    ...
    for (const account of filtered) {
      try {
        ...
        const result = await client.getDomesticBalance(
          account.account_no,
          productCode,
        );
        results.push({
        ...
        console.error(
          `Domestic balance failed [${maskAccountNo(account.account_no)}]:`,
          sanitized.message,
        );
      }
    }
