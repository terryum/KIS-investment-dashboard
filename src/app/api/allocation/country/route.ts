...
import { calculateCountryAllocation } from '@/lib/allocation/country';
import type { AllocationItem } from '@/lib/allocation/types';

/**
 * GET /api/allocation/country
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = calculateCountryAllocation(items);
    return NextResponse.json({ data: result });
  } catch {
    return NextResponse.json(
      { error: 'Failed to calculate country allocation' },
      { status: 500 },
    );
  }
}
