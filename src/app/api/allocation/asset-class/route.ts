...
import { calculateAssetClassAllocation } from '@/lib/allocation/asset-class';
import type { AllocationItem } from '@/lib/allocation/types';

/**
 * GET /api/allocation/asset-class
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = calculateAssetClassAllocation(items);
    return NextResponse.json({ data: result });
  } catch {
    return NextResponse.json(
      { error: 'Failed to calculate asset class allocation' },
      { status: 500 },
    );
  }
}
