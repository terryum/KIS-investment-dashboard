...
import { calculateStockStructure } from '@/lib/allocation/stock-analysis';
import type { AllocationItem } from '@/lib/allocation/types';

/**
 * GET /api/allocation/stock-structure
 * Stock structure breakdown (broad/factor/sector/theme/individual).
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = calculateStockStructure(items);
    return NextResponse.json({ data: result });
  } catch {
    return NextResponse.json(
      { error: 'Failed to calculate stock structure' },
      { status: 500 },
    );
  }
}
